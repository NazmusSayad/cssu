...
import { initDatabase, closeDatabase } from '../src/evaluator.js';
...
describe('Integration Tests', () => {
  ...
  async function fetchPost(path: string, body: any) {
  ...
  it('should create new user', async () => {
    const res = await fetchPost('/users', { name: 'Jane', email: 'jane@example.com' });
    expect(res.status).toBe(200);
