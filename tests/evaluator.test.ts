...
import { evaluateExpression, initDatabase, closeDatabase } from '../src/evaluator.js';
...
describe('Evaluator', () => {
  ...
  describe('SQL expressions', () => {
    beforeEach(() => {
      ...
      `);
      testDb.exec(`INSERT INTO users (name, email) VALUES ('John', 'john@example.com')`);
      testDb.exec(`INSERT INTO users (name, email) VALUES ('Jane', 'jane@example.com')`);
      testDb.close();
      testDb = null;
    ...
    it('should execute INSERT', () => {
      ...
      const result = evaluateExpression(expr, ctx) as any;
  ...
  describe('If expressions', () => {
    it('should return first truthy branch', () => {
      ...
      const expr: Expression = {
        type: 'if',
        branches: [
          { condition: { type: 'truthy', varName: 'user' }, value: { type: 'literal', value: 'found' } },
        ],
    ...
    it('should evaluate equals condition', () => {
      ...
      const expr: Expression = {
        type: 'if',
        branches: [
          { condition: { type: 'equals', varName: 'role', value: 'admin' }, value: { type: 'literal', value: 'granted' } },
        ],
    ...
    it('should evaluate notEquals condition', () => {
      ...
      const expr: Expression = {
        type: 'if',
        branches: [
          { condition: { type: 'notEquals', varName: 'status', value: 'banned' }, value: { type: 'literal', value: 'ok' } },
        ],
    ...
    it('should evaluate greaterThan condition', () => {
      ...
      const expr: Expression = {
        type: 'if',
        branches: [
          { condition: { type: 'greaterThan', varName: 'age', value: 18 }, value: { type: 'literal', value: 'adult' } },
        ],
    ...
    it('should evaluate lessThan condition', () => {
      ...
      const expr: Expression = {
        type: 'if',
        branches: [
          { condition: { type: 'lessThan', varName: 'age', value: 13 }, value: { type: 'literal', value: 'child' } },
        ],
    ...
    it('should evaluate NOT condition', () => {
      ...
      const expr: Expression = {
        ...
        branches: [
          {
            condition: { type: 'not', condition: { type: 'truthy', varName: 'blocked' } },
    ...
    it('should evaluate multiple branches', () => {
      ...
      const expr: Expression = {
        type: 'if',
        branches: [
          { condition: { type: 'equals', varName: 'role', value: 'admin' }, value: { type: 'literal', value: 'full access' } },
          { condition: { type: 'equals', varName: 'role', value: 'moderator' }, value: { type: 'literal', value: 'limited access' } },
        ],
  ...
  describe('Truthy checks', () => {
    ...
    it('should treat undefined as falsy', () => {
      ...
      const expr: Expression = {
        type: 'if',
        branches: [{ condition: { type: 'truthy', varName: 'x' }, value: { type: 'literal', value: 'yes' } }],
        elseValue: { type: 'literal', value: 'no' },
      };
      expect(evaluateExpression(expr, ctx)).toBe('no');
    });
